...
StarterGui:SetCore("SendNotification", {
    Title = "drac.cc loading skid wait..",
...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("drac.cc | Security : You are running another script, please disable it and execute again")
    end
end
    ...
    LPH_JIT_MAX(function()
        ...
        if Game_Name == "South Bronx" then
            ...
            HideUI = LPH_NO_VIRTUALIZE(function(Title, Timing)
                ...
                textLabel.Text = '<font color="rgb(0,163,224)">drac.</font>.cc\n' .. Title
...
-- UI Accent Override (Purple) [SAFE]
task.defer(function()
    pcall(function()
        if library then
            library.accent = Color3.fromRGB(170, 0, 255)
            if library.theme then
                library.theme.accent = Color3.fromRGB(170, 0, 255)
                library.theme.text = Color3.fromRGB(170, 0, 255)
            end
        end
    end)
end)
...
local window = library:window({name = "drac", suffix = ".cc", gameInfo = string.format("drac.cc : %s", Game_Name:lower())})
    ...
    window:seperator({name = "Game"}) do
        local LocalPlayerTab, PlayersTab, PurchaseGunTab, MiscTab, SafeTab = window:tab({name = "Main", tabs = {"Local Player", "Players", "Teleports", "Misc", "Safe"}, icon = GetImage("World.png")}) do
            ...
            do -- \\ Misc Tab
                ...
                DupingSection:label({wrapped = true, name = "This might bug if you have more than 1 of the item you're duping idiot!"})
                ...
                VulnerabilitySection:button({name = "Generate Max Illegal Money", callback = function()
                    ...
                    if Cup and Found then
                        HideUI("drac.cc generating illegal cash 💷\n please wait.")
...
task.spawn(function()
	...
	repeat
		task.wait()
		mainGui = game.CoreGui:FindFirstChild("drac.cc", true)
	until mainGui and mainGui:IsA("Frame")

	-- Prevent duplicate glow
	if mainGui.Parent:FindFirstChild("PurpleGlow") then return end

	-- Glow frame
	local glow = Instance.new("Frame")
	glow.Name = "PurpleGlow"
	glow.Parent = mainGui.Parent
	glow.ZIndex = mainGui.ZIndex - 1
	glow.AnchorPoint = mainGui.AnchorPoint
	glow.Position = mainGui.Position
	glow.Size = mainGui.Size + UDim2.fromOffset(18, 18)

	glow.BackgroundColor3 = Color3.fromRGB(170, 70, 255)
	...
	stroke.Parent = glow
	stroke.Thickness = 14
	stroke.Color = Color3.fromRGB(185, 90, 255)
